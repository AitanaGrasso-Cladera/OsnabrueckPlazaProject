...
for sub = 1:length(participants)
    ...
    %% Save the data
    if allEvents == 1
        save(fullfile(savedata,sprintf('betasUnfoldFinalAllEvents_%s.mat',participants(sub).name)),'ufresult','ufresultEp');
    elseif startEvents == 1
         save(fullfile(savedata,sprintf('betasUnfoldFinalStartEvents_%s.mat',participants(sub).name)),'ufresult','ufresultEp');
    elseif endEvents == 1
         save(fullfile(savedata,sprintf('betasUnfoldFinalEndEvents_%s.mat',participants(sub).name)),'ufresult','ufresultEp');
    elseif middleEvents == 1
         save(fullfile(savedata,sprintf('betasUnfoldFinalMiddleEvents_%s.mat',participants(sub).name)),'ufresult','ufresultEp');
    end
end
if allEvents == 1
    save(fullfile(savedata,sprintf('betasUnfoldALL_overlapFinalAllEvents.mat')),'sacc','blinkOn','fix','time','blinkOff',"allAIC","allBIC");
elseif startEvents == 1
    save(fullfile(savedata,sprintf('betasUnfoldALL_overlapFinalStartEvents.mat')),'sacc','blinkOn','time',"allAIC","allBIC");
elseif endEvents == 1
    save(fullfile(savedata,sprintf('betasUnfoldALL_overlapFinalEndEvents.mat')),'fix','blinkOff','time',"allAIC","allBIC");
elseif middleEvents == 1
    save(fullfile(savedata,sprintf('betasUnfoldALL_overlapFinalMiddleEvents.mat')),'saccMiddle','blinkMiddle','time',"allAIC","allBIC");
end
%%
