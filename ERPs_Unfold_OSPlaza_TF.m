...
% July 26 - AGC = Modified the script to deconvolute different models based
% on distinct target events.
% July 26 - DN - AGC =  Added AIC and BIC computation to perform model
% selection.
%% Path definition
projectFolder = [];
...
clear tmp
%% Define which model you are working on
startEvents = 0;
endEvents = 0;
middleEvents = 0;
allEvents = 0;
...
for sub = 1:length(participants)
    ...
    elseif startEvents == 1
        EEG = pop_loadset(sprintf('2a_cleanDataChannels_noRejectionStart_%s.set',participants(sub).name(9:end)),fullfile([dataFolder,filesep,participants(sub).name]));
    elseif endEvents == 1
        EEG = pop_loadset(sprintf('2a_cleanDataChannels_noRejectionEnd_%s.set',participants(sub).name(9:end)),fullfile([dataFolder,filesep,participants(sub).name]));
    elseif middleEvents == 1
        EEG = pop_loadset(sprintf('2a_cleanDataChannels_noRejectionMiddle_%s.set',participants(sub).name(9:end)),fullfile([dataFolder,filesep,participants(sub).name]));
    end
    ...
    elseif startEvents == 1
        EEGchan = pop_loadset(sprintf('1a_triggersFilteredStart_%s.set',participants(sub).name(9:end)),fullfile([dataFolder,filesep,participants(sub).name]));
    elseif endEvents == 1
        EEGchan = pop_loadset(sprintf('1a_triggersFilteredEnd_%s.set',participants(sub).name(9:end)),fullfile([dataFolder,filesep,participants(sub).name]));
    elseif middleEvents == 1
        EEGchan = pop_loadset(sprintf('1a_triggersFilteredMiddle_%s.set',participants(sub).name(9:end)),fullfile([dataFolder,filesep,participants(sub).name]));
    end
    ...
    elseif startEvents == 1
        cfgDesign.eventtypes    = {'saccadeOnset','blinkOnset'};
        cfgDesign.formula       = {'y ~ 1','y ~ 1'};
    elseif endEvents == 1
        cfgDesign.eventtypes    = {'fixationOnset','blinkOffset'};
        cfgDesign.formula       = {'y ~ 1','y ~ 1'};
    elseif middleEvents == 1
        cfgDesign.eventtypes    = {'middleSaccade','middleBlink'};
        cfgDesign.formula       = {'y ~ 1','y ~ 1'};
    end
   ...
   %% Compute AIC and BIC
   B = reshape(permute(EEG.unfold.beta_dc, [2 3 1]), [], size(EEG.data,1));
   Yhat = EEG.unfold.Xdc * B;
   Y = EEG.data.';
   R = Y - Yhat;
   RSS = sum(R(:).^2,1);   % one RSS per channel
   n = numel(Y);
   %k   = rank(EEG.unfold.Xdc);
   k = size(EEG.unfold.Xdc,2);
   AIC = n.*log(RSS/n) + 2*k;
   ...
   %% Make a massive uni-variate fit without de-convolution (Gert et al., 2022)
    ...
    elseif startEvents == 1
        ...
        blinkOn(cnt,:,:)  = ufresultEp.beta(:,:,paramBlinkOnset);
    elseif endEvents == 1
        [~,paramFixation]=find(ismember(paramNames,'(Intercept)'));
        [~,paramBlinkOffset]=find(ismember(paramNames,'2_(Intercept)'));

        fix(cnt,:,:) = ufresultEp.beta(:,:,paramFixation);
        blinkOff(cnt,:,:)  = ufresultEp.beta(:,:,paramBlinkOffset);
    elseif middleEvents == 1
        [~,paramMiddleSaccade]=find(ismember(paramNames,'(Intercept)'));
        [~,paramMiddleBlink]=find(ismember(paramNames,'2_(Intercept)'));

        saccMiddle(cnt,:,:) = ufresultEp.beta(:,:,paramMiddleSaccade);
        blinkMiddle(cnt,:,:)  = ufresultEp.beta(:,:,paramMiddleBlink);
    end
    ...
    %% Save the data
    if allEvents == 1
        save(fullfile(savedata,sprintf('betasUnfoldFinalAllEventsTF_%s.mat',participants(sub).name)),'ufresult','ufresultEp');
    elseif startEvents == 1
         save(fullfile(savedata,sprintf('betasUnfoldFinalStartEventsTF_%s.mat',participants(sub).name)),'ufresult','ufresultEp');
    elseif endEvents == 1
         save(fullfile(savedata,sprintf('betasUnfoldFinalEndEventsTF_%s.mat',participants(sub).name)),'ufresult','ufresultEp');
    elseif middleEvents == 1
         save(fullfile(savedata,sprintf('betasUnfoldFinalMiddleEventsTF_%s.mat',participants(sub).name)),'ufresult','ufresultEp');
    end
end

if allEvents == 1
    save(fullfile(savedata,sprintf('betasUnfoldALL_overlapFinalAllEventsTF.mat')),'sacc','blinkOn','fix','time','blinkOff',"allAIC","allBIC");
elseif startEvents == 1
    save(fullfile(savedata,sprintf('betasUnfoldALL_overlapFinalStartEventsTF.mat')),'sacc','blinkOn','time',"allAIC","allBIC");
elseif endEvents == 1
    save(fullfile(savedata,sprintf('betasUnfoldALL_overlapFinalEndEventsTF.mat')),'fix','blinkOff','time',"allAIC","allBIC");
elseif middleEvents == 1
    save(fullfile(savedata,sprintf('betasUnfoldALL_overlapFinalMiddleEventsTF.mat')),'saccMiddle','blinkMiddle','time',"allAIC","allBIC");
end
%%
